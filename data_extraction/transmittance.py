...
def transmittance_scan(data_dir):

    df = None
    for file_no, file in enumerate(sorted(glob.glob(data_dir + '/*/*Sample.Raw.csv'))):

        print('file name', os.path.basename(file), "ppeye001-01-01.Sample.Raw.csv".strip("Sample.Raw.csv"))
        column_name = os.path.basename(file).strip('Sample.Raw.csv')
